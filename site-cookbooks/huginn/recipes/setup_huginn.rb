deploy_revision node['huginn']['deploy_user']['home'] do
  ...
  before_migrate do
    ...
    # Pull in various attributes to the environment hash
    environment_options["APP_SECRET_TOKEN"] = "$(cat '#{new_resource.deploy_to}/shared/rakesecret')"
    environment_options["DATABASE_NAME"] = node['huginn']['database']['name']
    environment_options["DATABASE_USERNAME"] = node['huginn']['database']['username']
    environment_options["DATABASE_PASSWORD"] = node['huginn']['database']['password']
    environment_options["DATABASE_PORT"] = node['huginn']['database']['port']
    environment_options["DATABASE_POOL"] = node['huginn']['database']['pool']
    environment_options["DATABASE_ADAPTER"] = node['huginn']['database']['adapter']
    environment_options["DATABASE_ENCODING"] = node['huginn']['database']['encoding']
    environment_options["DATABASE_RECONNECT"] = node['huginn']['database']['reconnect']
    environment_options["DATABASE_HOST"] = node['huginn']['database']['host']
    environment_options["DATABASE_SOCKET"] = node['huginn']['database']['socket']
    environment_options["RBENV_VERSION"] = node['huginn']['ruby_version']

    # Then override any default/example/attribute values with those set by the
    # user
    environment_options.merge! node['huginn']['env']

    file "#{release_path}/shared/dotenv" do
      content <<-EOENV
